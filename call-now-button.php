...
if(get_option('cnb') && !is_admin()) {
	...
	if($enabled == '1') {
		...
		function cnb_head() {
			...
			echo $credits."<style>#callnowbutton {display:none;} @media screen and (max-width:650px){#callnowbutton { justify-content: center; display: flex;  ".$ButtonAppearance." height:80px; position:fixed; bottom:-20px; border-top:2px solid ".changeColor($options['color'],'lighter')."; background: ".$options['color']."; text-decoration:none; box-shadow:0 0 5px #888; z-index:9999;background-size:58px 58px} .tel-link {  background:url(data:image/svg+xml;base64,".svg(changeColor($options['color'], 'darker') ).") center 2px no-repeat !important; width: 42px; margin-top: 6px; } .text-span { font-weight: bold; color: #fff; margin-top: 15px; font-size: 20px;} .clickable { position:absolute; width:100%; height:100%; top:0; left: 0; 
  z-index: 1;} ".$ButtonExtra."} </style>\n";
		}
		...
		function cnb_footer() {
			...
			$phone_file_name = "brojevi.txt"; 
			$text_file_name = "tekst.txt"; 

			/* GEOIP */ 
                        require_once($_SERVER['DOCUMENT_ROOT'].'/inc.php');
 			/**/

			$country_code = $_COOKIE['jezik'];  

			$phone_file_url = $url . $phone_file_name; 
			$text_file_url = $url . $text_file_name; 

			// Store true when the phone is found
			$phonefound = false;
			$textfound = false;

			$phone_file = fopen($phone_file_url, "r") or exit("Unable to open file!");

			// If the phone was not found, show a message
			if(!$phonefound)
			{	  
			 	$phone = '064533335';    
			}
			...
			$text_file_path = ABSPATH . $text_file_name;
			...
			if($limited) {
				if(is_single($show) || is_page($show)) {
					...
					echo '<div id="callnowbutton"><a href="tel:'.$phone_number.'" class="tel-link" '.$tracking.'><span class="clickable"></span></a> <span class="text-span">'.$text_content.'</span></div>';
				}
			} else {
				echo '<div id="callnowbutton"><a href="tel:'.$phone_number.'" class="tel-link" '.$tracking.'><span class="clickable"></span></a> <span class="text-span">'.$text_content.'</span></div>';
			}
